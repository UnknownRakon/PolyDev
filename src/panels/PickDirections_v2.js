...
class PickDirections_v2 extends Component {
  ...
  componentDidMount = () => {
    if (localStorage.getItem("selectedGroups")) {
      ...
      for (
        let i = 0;
        i < document.querySelectorAll("input[type=checkbox]").length * 2 - 1;
        i++
      ) {
        if (localStorage.getItem("selectedGroups")[i] !== ",") {
          document.querySelectorAll("input[type=checkbox]")[counter].checked =
            localStorage.getItem("selectedGroups")[i] === "t" ? true : false;
          counter = counter + 1;
        }
      }
  ...
  handleInfoClick = (value) => (event) => {
    ...
    for (
      let i = 0;
      i < document.querySelectorAll("input[type=checkbox]").length;
      i++
    ) {
      this.state.selectedGroups.push(
        document.querySelectorAll("input[type=checkbox]")[i].checked ? "t" : "f"
      );
    }
  ...
  goNextPageHandler = () => (event) => {
    ...
    for (
      ...
      if (document.querySelectorAll("input[type=checkbox]")[i].checked) {
        this.state.choosedGroupTempArray.push(
          document.querySelectorAll("input[type=checkbox]")[i].value
        );
      }
    }
  ...
  render() {
    return (
      <Panel id={this.props.id}>
        <PanelHeader
          left={
            <PanelHeaderBack onClick={this.props.go} data-to="acquaintance" />
          }
        >
          PolyApp
        </PanelHeader>
                ...
                return (
                  ...
                  >
                    <FormItem className="directions__checkbox">
                      <Checkbox
                        value={direction["Название направления"]}
                        checked={this.value}
                        onChange={() => this.handleCheckboxChange(event)}
                      >
                        {direction["Название направления"]}
                      </Checkbox>
                    </FormItem>
                    <Icon20Info
                      onClick={this.handleInfoClick(JSON.stringify(direction))}
        ...
        <FixedLayout filled className="pickDirections__fixed" vertical="bottom">
          <Div>
            <Button
              ...
              onClick={this.goNextPageHandler(event)}
