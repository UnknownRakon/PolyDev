    ...
    return (
        <Panel id={id}>
            ...
            <List>
                ...
                <Cell indicator={interestedDirection['Срок обучения']}>Срок обучения</Cell>
                <Cell indicator={interestedDirection['Бюджетных мест']}>Бюджетных мест</Cell>
                <Cell indicator={interestedDirection['Платных мест']}>Платных мест</Cell>
            </List>
            <Text weight="medium" style={{ marginBottom: 0, padding: 16, paddingBottom: 0 }} dangerouslySetInnerHTML={createMarkup(interestedDirection['Описание'])}></Text>
            <Div style={{ paddingBlockEnd: 0, paddingBlockStart: 0 }}><Link className='link-instruction' href={interestedDirection['Учебный план']} target="_blank">Учебный план</Link></Div>
            <Div style={{ paddingBlockEnd: 0, paddingBlockStart: 0, marginBlockEnd: 70 }}><Link className='link-instruction' href={interestedDirection['Карта направления']} target="_blank">Карта направления</Link></Div>
            <FixedLayout filled vertical="bottom">
                <Div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center!important' }}>
                    <Link style={{
                        width: '100%', textAlign: 'center', color: "var(--button_primary_foreground)",
                        ...
                        href={interestedDirection['Подробнее']} target="_blank"> Подробнее</Link>
                </Div>
            </FixedLayout>
        </Panel>
    )
