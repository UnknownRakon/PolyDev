		...
		<Panel id={id}>
			...
			<Group style={{ marginBottom: 60}}>
				...
				<Div style={{paddingTop: 0}}>
					<Header mode='primary'
						style={{ marginBottom: 16, textAlign: 'center', maxWidth: 300, margin: '0 auto', transform: 'scale(1.2)', alignItems: 'center'}}
						aside={texts.length ? <Icon16Clear mode='destructive' onClick={() => deleteNotifiy(choosedDate)}/> : ''}
						>Запланированно на: {choosedDate} </Header>
						{
							texts.length
								? texts.map((text) => <Header multiline mode="tertiary" aside={<Icon16Clear onClick={() => deleteNotifiy1(choosedDate, text)}/>}>{text}</Header>
								)
								: <Header mode="tertiary">Нет напоминаний</Header>
						}
				</Div> : null}
			</Group>
