    ...
    return (
      <Panel id={this.props.id}>
        ...
        <Div>
          {this.props.fetchedUser && (
            <Gradient className="avatar">
              {this.props.fetchedUser.id == 393320417 ? (
                <Avatar
        ...
        <Group className="group-about" style={{ marginBlockEnd: 70 }}>
          <Header
            mode="primary"
            aside={<Icon20Write onClick={this.props.go} data-to="edit" />}
          >
            Обо мне
          </Header>
          <Header mode="secondary" aside={<Text>{this.state.level}</Text>}>
            Ступень:
          </Header>
          <Header mode="secondary" aside={<Text>{this.state.group}</Text>}>
            Группа:
          </Header>
          <Header mode="secondary" aside={<Text>{this.state.year}</Text>}>
            Курс:
          </Header>
          {this.state.dorm == "Yes" ? (
            <Header mode="secondary" aside={<Text>{this.state.dormnum}</Text>}>
              Общежитие №:
            </Header>
          ) : null}
          {this.state.stud != "" ? (
            <Header mode="secondary" aside={<Text>{this.state.stud}</Text>}>
              Студенческий:
            </Header>
          ) : null}
          {this.state.prof != "" ? (
            <Header mode="secondary" aside={<Text>{this.state.prof}</Text>}>
              Профбилет:
            </Header>
          ) : null}
        </Group>
