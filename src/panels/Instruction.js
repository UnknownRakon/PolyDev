...
const Instruction = ({ id, category, question, go, back }) => {
    ...
    return (
        <Panel id={id}>
            ...
            <Div>
                {category == 'dorms' ? <Icon36HomeOutline className="instruction__icon" /> : null}
                {category == 'study' ? <Icon28BookOutline className="instruction__icon" /> : null}
                {category == 'buildings' ? <Icon56SchoolOutline className="instruction__icon" /> : null}
                {category == 'PD' ? <Icon28LightbulbStarOutline className="instruction__icon" /> : null}
                <Title style={{ marginTop: 10, textAlign: 'center' }} level="2" weight="medium">{question.question}</Title>
            </Div>
            <Group className="group-about" style={{ marginBottom: 130 }}>
                <Div style={{ paddingBlockEnd: 0 }}><Headline weight="regular" dangerouslySetInnerHTML={createMarkup(question.instruction)}></Headline></Div>
                {question.links.length !== 0 ? <Header className='heading' style={{ padding: 0 }} mode="secondary">Полезные ссылки</Header> : null}
                {question.links !== 0 &&
