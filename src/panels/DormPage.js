...
const DormPage = ({ id, back, dorm, setdorm }) => {
  ...
  return (
    <Panel id={id}>
      ...
      <Title level="1" weight="regular" className="dorm__title">Общежитие {dorms.Общежития[`${dorm}`].Номер} </Title>
      <Group>
        <Gallery
          slideWidth="90%"
          style={{ height: 300 }}
      ...
      <Div style={{ margin: '0 16px 8px 16px', lineHeight: '1.4rem', marginBlockEnd: 70 }}>
        <Text dangerouslySetInnerHTML={createMarkup(dorms.Общежития[dorm].Описание)} />
      </Div>
      <FixedLayout filled vertical="bottom">
        <Div style={{ display: "flex", justifyContent: "space-between" }}>
          <Button style={{ width: "40%" }} size="l" onClick={goBackPageHandler} data-to="choosed-directions-info">Предыдущее</Button>
          <Button style={{ width: "40%" }} size="l" onClick={goNextPageHandler} data-to="choosed-directions-info">Следующее</Button>
        </Div>
      </FixedLayout>
    </Panel>
  )
